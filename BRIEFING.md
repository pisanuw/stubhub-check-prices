...
- Current scope: Personal, single-event, intentionally fragile. Playwright +
  real Chrome loads the page; prices parsed from the server-rendered
  `<script id="index-data">` JSON blob. Output to CSV + log. Scheduled via
  launchd every 15 min. Email alerts (Resend) on >=20% price drop vs previous
  reading OR a category appearing for the first time, across ALL categories
  (classes + sections + watched named products: Champions Club, Hospitality,
  Trophy Lounge, FIFA Pavilion).
- Key decisions:
